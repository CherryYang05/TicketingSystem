...
public class Replay {
	...
	private static boolean readConfig(String filename) {
		try {
			...
			while (scanner.hasNextLine()) {
				...
				System.out.println("route: " + routenum + ", coach: " + coachnum + ", seatnum: " + seatnum
						+ ", station: " + stationnum + ", refundRatio: " + refRatio + ", buyRatio: " + buyRatio
						+ ", inquiryRatio: " + inqRatio);
				linescanner.close();
			}
			scanner.close();
		...
		if (methodName.equals("buyTicket")) {
			if (line.res.equals("false")) {
				...
				if (num == 0)
					...
					System.out.println("Error: TicketSoldOut" + " " + line.pretime + " " + line.posttime + " "
							+ line.threadid + " " + line.route + " " + line.departure + " " + line.arrival);
					System.out.println(
							"RemainTicket" + " " + num + " " + line.route + " " + line.departure + " " + line.arrival);
					return false;
				}
			}
			...
			if (ticket1 != null && line.res.equals("true") &&
				...
				System.out.println("Error: Ticket is bought" + " " + line.pretime + " " + line.posttime + " "
						+ line.threadid + " " + ticket.tid + " " + ticket.passenger + " " + ticket.route + " "
			...
			if ((flag && line.res.equals("true")) || (!flag && line.res.equals("false")))
				...
				System.out.println("Error: Ticket is refunded" + " " + line.pretime + " " + line.posttime + " "
						+ line.threadid + " " + ticket.tid + " " + ticket.passenger + " " + ticket.route + " "
						+ ticket.coach + " " + ticket.departure + " " + ticket.arrival + " " + ticket.seat);
				return false;
			}
			...
			if (num == line.seat)
				...
				System.out
						.println("Error: RemainTicket" + " " + line.pretime + " " + line.posttime + " " + line.threadid
								+ " " + line.route + " " + line.departure + " " + line.arrival + " " + line.seat);
				System.out.println("Real RemainTicket is" + " " + line.seat + " " + ", Expect RemainTicket is" + " "
						+ num + ", " + line.route + " " + line.departure + " " + line.arrival);
				return false;
			}
		}
	...
	private static void writeline(ArrayList<HistoryLine> historyList, int line) {
		...
		System.out.println(tl.pretime + " " + tl.posttime + " " + tl.threadid + " " + tl.operationName + " " + tl.tid
				+ " " + tl.passenger + " " + tl.route + " " + tl.coach + " " + tl.departure + " " + tl.arrival + " "
				+ tl.seat);
	}
	...
	private static boolean checkline(ArrayList<HistoryLine> historyList, int index) {
		...
		for (int i = 0; i < methodList.size(); i++) {
			if (line.operationName.equals(methodList.get(i))) {
				...
				// System.out.println("Line " + index + " executing " + methodList.get(i) + "
				// res: " + flag + " tid = " + line.tid);
				return flag;
			}
		}
		return false;
